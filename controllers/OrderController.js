...
exports.orderList = [
	...
	function (req, res) {
		try {
			Order.find({user: req.user._id},"_id title description isbn createdAt").then((orders)=>{
				if(orders.length > 0){
...
exports.orderDetail = [
	...
	function (req, res) {
		...
		try {
			Order.findOne({_id: req.params.id,user: req.user._id},"_id title description isbn createdAt").then((order)=>{                
				if(order !== null){
